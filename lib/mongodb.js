...
export async function getKitStock(kitId) {
  const db = await getDb();
  const record = await db.collection("kitStock").findOne({ kitId });
...
export async function setKitStock(kitId, stock) {
  const db = await getDb();
  if (stock === null) {
    await db.collection("kitStock").deleteOne({ kitId });
    return null;
  }
  await db.collection("kitStock").updateOne(
    { kitId },
    {
      ...
      $setOnInsert: { kitId, createdAt: new Date() },
    },
    { upsert: true }
  );
  return stock;
...
export async function decrementKitStock(kitId) {
  const db = await getDb();
  const existing = await db.collection("kitStock").findOne({ kitId });
